...
class SpallSourceContractTests(unittest.TestCase):
    ...
    def test_damage_queries_use_path_specific_bounded_candidate_snapshots(self):
        ...
        self.assertIn("function ACF_SortDamageCandidates(", self.source)
        self.assertIn("local Parent = math.floor(At / 2)", self.source)
        self.assertIn("if Distance > ChildDistance or (Distance == ChildDistance and EntityIndex >= ChildIndex) then break end", self.source)
    ...
    def test_scaled_explosion_iterates_a_snapshot_before_live_registry_removal(self):
        ...
        self.assertIn("local ExplosionPending = {}", self.source)
        self.assertIn("for Found in pairs(ExplosionPending) do", self.source)
        self.assertIn("ACF_InsertNearestDamageCandidate(PendingExplosives", self.source)
        self.assertIn("ACF_InsertNearestDamageCandidate(FreshExplosives", self.source)
        self.assertIn("local FreshExplosives = {}", self.source)
        self.assertIn("if RemainingCandidates > 0 then", self.source)
        self.assertIn("local PendingAt = 1", self.source)
        self.assertIn("local FreshAt = 1", self.source)
        self.assertIn("if Owner ~= EOwner then continue end", self.source)
        self.assertIn("ExplosionPending[Found] = true", self.source)
        self.assertIn("ExplosionSeen[Found] = true", self.source)
        self.assertIn("RadiusSq = Radius ^ 2", self.source)
        self.assertNotIn("NewExplosives", self.source)
    ...
    def test_ballistics_scheduler_uses_pooled_state_and_active_iteration(self):
        ...
        self.assertIn("local CurrentActiveSlot = 0", source)
        self.assertIn("while Slot <= ActiveCount do", source)
        self.assertIn("if CurrentActiveSlot < Slot then", source)
        self.assertIn("Bullet.ActiveFrame ~= Frame", source)
        self.assertIn("BulletData.ActiveFrame = CurrentBallisticsFrame", source)
        self.assertIn("ACE.BallisticsFrame = CurrentBallisticsFrame", source)
        self.assertIn("local Frame = CurrentBallisticsFrame", source)
    ...
    def test_ballistics_work_and_debug_limits_are_explicit(self):
        ...
        self.assertIn("BulletData.Index\t\t= ACF.CurBulletIndex", missile)
        self.assertIn("local ActiveBullet = ACF_AcquireBullet(BulletData)", missile)
        self.assertIn("ACF_RegisterBullet(ACF.CurBulletIndex, ActiveBullet)", missile)
    ...
    def test_explosive_registry_removal_keeps_its_index_map_consistent(self):
        ...
        self.assertIn("ACE.RemoveExplosive(Ent)", contraption)
        self.assertIn("ACE.Explosives        = {}", contraption)
        self.assertIn("ACE.ExplosiveIndex    = {}", contraption)
        self.assertNotIn("for _, explosive in ipairs(ACE.Explosives)", contraption)
